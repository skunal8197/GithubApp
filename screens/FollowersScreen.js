...
import { View, StyleSheet, FlatList, Linking, Button, AsyncStorage } from 'react-native';
...
export default class FollowersScreen extends React.Component {
 ...
 /*
 Gets user followers information from the GitHub API
 */
 getUserFollowers = () => {
    const url = `https://api.github.com/users/skunal8197/followers`;
    ...
    fetch(url, {
      ...
      .then(res => {
        this.storeFollowersList()
      })
      .then(res => {
        console.log("Storing my followers list in AsyncStorage")
        this.retrieveData('followers')
      })
      .catch(error => {
  ...
  */
  ...
  storeFollowersList = () => {
    followersList = []
    for(var i = 0; i < this.state.data.length; i++){
      followersList.push(this.state.data[i]["login"])
    }

    this.storeData('followers', JSON.stringify(followersList))
  }
