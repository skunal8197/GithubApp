 ...
 */
 getUserFollowing = () => {
    const url = `https://api.github.com/users/skunal8197/following`;
    ...
    fetch(url, {
      ...
      .then(res => {
        this.storeFollowingList()
      })
      .then(res => {
        console.log("Storing my following list in AsyncStorage")
        this.retrieveData('following')
      })
      .catch(error => {
  ...
  */
  ...
  storeFollowingList = () => {
    followingList = []
    for(var i = 0; i < this.state.data.length; i++){
      followingList.push(this.state.data[i]["login"])
    }

    this.storeData('following', JSON.stringify(followingList))
  }
